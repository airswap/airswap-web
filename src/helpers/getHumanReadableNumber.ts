...
export const getHumanReadableNumber: (
    ...
    if(beforeDecimalPoint.length < 4 || beforeDecimalPoint == "0" ){
        ...
        //else have to trim
            ...
            readableNumber = beforeDecimalPoint + "." + afterDecimalPoint.substring(0,iter+numSigDigits);
        ...
        if(beforeDecimalPoint.length >= 7){
            readableNumber = beforeDecimalPoint.substring(0,beforeDecimalPoint.length - (7 - (numSigDigits-1)));
            //check if after decimal point is significant
            if(beforeDecimalPoint[beforeDecimalPoint.length - (7 - (numSigDigits-1))] !== '0'){
                readableNumber += ".";
                readableNumber += beforeDecimalPoint[beforeDecimalPoint.length - (7 - (numSigDigits-1))];
            }
            readableNumber += "M";
            return readableNumber;
        }
        ...
        if(beforeDecimalPoint.length >= 4){
            readableNumber = beforeDecimalPoint.substring(0,beforeDecimalPoint.length - (4 - (numSigDigits-1)));
            //check if after decimal point is significant
            if(beforeDecimalPoint[beforeDecimalPoint.length - (4 - (numSigDigits-1))] !== '0'){
                readableNumber += ".";
                readableNumber += beforeDecimalPoint[beforeDecimalPoint.length - (4 - (numSigDigits-1))];
            }
            readableNumber += "k";
            return readableNumber;
        }
